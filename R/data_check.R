data_check <- function(out){
  ...
  zer_rows <- apply(X,1,sum)
  zer_cols <- apply(X,2,sum)
  ...
  if (ncol(X) < 4) stop("The number of items in the data should be larger than four.")
  
  ####################### CALCULATE ITEM DESCRIPTIVES ####################
  ##############################################################################
  
  out_list$DESCRIPTIVES <- list()
  out_list$DESCRIPTIVES$positives_per_item <- apply(X,2,sum)
  out_list$DESCRIPTIVES$positives_per_person <- apply(X,1,sum)
  out_list$DESCRIPTIVES$n_items_final <- ncol(X)
  out_list$DESCRIPTIVES$item_names_final <- colnames(X)
  out_list$DESCRIPTIVES$n_persons_final <- nrow(X)
  ...
  out_list$MUDFOLD_INFO$triple_stats$Expected_errors <- EO <- Err_exp(X) # Expected errors for triples
  out_list$MUDFOLD_INFO$triple_stats$Observed_errors <- O <- Err_obs(X) # Observed errors for triples
  out_list$MUDFOLD_INFO$triple_stats$H_coefficients <- 1 - (O/ EO) # H coefficients for triples
  start <- out_list$CALL$start
  if (!is.null(start) & any(!start %in% colnames(X))) stop("Starting vector should consist of items in the colnames(data).")
  ############################# CHECK FOR BINARY DATA ##########################
  ##############################################################################
  if(any(!(X==0 | X==1))) stop("Data should be binary. Consider using the function pick().") 
  ...
  return(list(data=data.frame(X),out=out_list))
}